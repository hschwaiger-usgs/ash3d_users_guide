...
Note that the gradient Richardson number given above is an appoximation to the flux
...
horizontal velocity through a mixing length, $\ell_c$. % Randerson %Eq.5.23, Monin 5.112,Stull 6.4.4f
...
$\mathcal{F}(\mathrm{Ri}) = (\mathrm{Ri}_c-\mathrm{Ri})/\mathrm{Ri}$ \cite{Jacobson}, %Jacobson Eq.8.70
