...
\end{equation}
which reduces $\ell_m$ near the surface from the free atmosphere mixing length, $\lambda_m$.

%\begin{equation}
%\ell_m = \frac{\kappa z}{1+ \frac{\kappa z}{\lambda_m}}
%\end{equation}
The stability function, $\mathcal{F}(\mathrm{Ri})$, supresses vertical diffusion for stable and weakly
unstable conditions, while promoting vertical diffusion in unstable conditions ($\mathrm{Ri}<0$).
...
A simplified form given in \cite{Jacobson2005,Stull1988} is given by:
...
One proposed by \cite{Betts1996} and used with the NCEP-NCAR models is given by:
...
Mixing Length as a function of the free-atmosphere mixing length (left). Function forms of $\mathcal{F}(\mathrm{Ri})$ implemented
in Ash3d.
}}
\end{figure}
...
\end{equation}
Businger and Arya (1974) propose this formulation using $h_{BL}=\frac{u_*}{8 f}$, essentially
usin an ABL from the Ekman layer (\ref{VarDiff_Eq_ABL2_1} with $c_n=0.125$).
%In this equation, $f$ is the Coriolis parameter described above and $u_*/f$ is
%the Ekman layer height.
